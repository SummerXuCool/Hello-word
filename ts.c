...
*	Review: 
******************************************************************************/
void mpegts_timer_mpts(TS_OPTIONS_T **glopts,  RNGBUF_t ** pVidBuf, RNGBUF_t ** pAudBuf, RNGBUF_t ** pInsertBuf, uint32_t *pChnEn, uint32_t ChnNum)
{
	long long	pcr = 0;
	long long 	current_system_time = 0;
	long long	tx_period = 0;
	...
	long long current_time = 0;
	int32_t		cc = 0;
	TS_OPTIONS_T *plChn0TsOpt; /* 通道 0 对象 */
	int i;

	for (i = 0; i < ChnNum; i++) {
		if ((pChnEn == NULL) || (glopts[i] == NULL)
			||(pVidBuf[i] == NULL) || (pAudBuf[i] == NULL) || (pInsertBuf[i] == NULL)) {
			return;
		}
	}

	plChn0TsOpt = glopts[0];
	if (plChn0TsOpt->syn_flag != 1)	{ /* PCR 同步标志 */
		return;
	}

	if (plChn0TsOpt->first_pcr == 0) { /* 初始化 PCR 值 */
		/* 给PCR赋初始值 */
		OS_TimeNow(&current_system_time);
		plChn0TsOpt->first_pcr = (current_system_time)*27; 

		printf("\r\n Init PCR (first_pcr)[%d] = %llu (os time = %llu)\n", plChn0TsOpt->m_pUserParam, plChn0TsOpt->first_pcr, current_system_time);
	}

	OS_TimeNow(&current_time);

	/* every 2s */
	if ((current_time - plChn0TsOpt->begin_calc_time) >= 2000000) {
		/* 调整PCR */
		if (plChn0TsOpt->syn_flag == 1) {
			mpegts_adjust_pcr_base(plChn0TsOpt);
		}

		plChn0TsOpt->begin_calc_time = current_time;
	}

	/* 得到发送时间周期 */
	if (plChn0TsOpt->previous_system_time == 0) {
		tx_period = 10;
	}
	else {
		tx_period = current_time - plChn0TsOpt->previous_system_time;
		tx_period = (long long)((double)tx_period /1000.0 + 0.5);
	}

	plChn0TsOpt->previous_system_time = current_time;

	/*计算应发送的TS 包数*/
	tx_ts_num = mpegts_get_ts_num(tx_period, plChn0TsOpt->mux_rate, &plChn0TsOpt->real_rate);

	/* 从buffer中去取帧*/
	frame_len = 0;
	for (i = 0; i < ChnNum; i++) {
		frame_len += RngNBytes(pVidBuf[i]) + RngNBytes(pAudBuf[i]) + RngNBytes(pInsertBuf[i]);
	}

	if(frame_len >= (TS_PACKET_SIZE*tx_ts_num)) { /* 不用填空包 */
		while(tx_ts_num > 0) {
			if (plChn0TsOpt->pcr_packet_count >= plChn0TsOpt->pcr_packet_period) {
				/* 插入PCR TS */
				plChn0TsOpt->pcr_packet_count = 0;

				for (i = 0; i < ChnNum; i++) {
					if (pChnEn[i]) {
						mpegts_get_pcr(plChn0TsOpt, &pcr);
						mpegts_tx_pcr_only(glopts[i], pcr); 

						plChn0TsOpt->ts_packet_count++;
						tx_ts_num--;
					}
				}
			}
			...
			for (i = 0; i < ChnNum; i++) {
				...
				if (RngNBytes(pInsertBuf[i]) >= TS_PACKET_SIZE) {
					lRet = RngBufGet(pInsertBuf[i], (char*)buf, TS_PACKET_SIZE);
				}
				else if (RngNBytes(pAudBuf[i]) >= TS_PACKET_SIZE) {
					lRet = RngBufGet(pAudBuf[i], (char*)buf, TS_PACKET_SIZE);
				}
				else {
					lRet = RngBufGet(pVidBuf[i], (char*)buf, TS_PACKET_SIZE);
				}

				if (TS_PACKET_SIZE == lRet) {
					cc = mpegts_get_cc_with_pid(buf, glopts[i]->ts_header.PID);
					if (cc != -1) {
						glopts[i]->last_video_cc = cc;
					}

					if (glopts[i]->send_ts_call != NULL) {
						glopts[i]->send_ts_call(buf, TS_PACKET_SIZE, glopts[i]->m_pUserParam);
					}
		...
		while(tx_ts_num > 0) {
			...
			if (read_ts_num > 0)
			{
				...
				/* 插入空包*/
				mpegts_insert_null_packet(buf, TS_PACKET_SIZE);
				if (plChn0TsOpt->send_ts_call != NULL) {
					plChn0TsOpt->send_ts_call(buf, TS_PACKET_SIZE, plChn0TsOpt->m_pUserParam);
				}
			}
...
static void mpegts_insert_pcr_only(TS_OPTIONS_T *glopts, LONGLONG pcr)
{
    ...
    *q++ = glopts->pcr_ts_opt.pcr_pid;
	...
	if (glopts->ts_header.PID == glopts->pcr_ts_opt.pcr_pid) /* 处理视频 PID 与 PCR PID 相同的情况 */
	{
